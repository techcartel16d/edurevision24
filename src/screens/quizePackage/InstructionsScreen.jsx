...
const InstructionsScreen = ({ route }) => {
    ...
    const { categoryId, testSeriesId, packgetId, data, total_marks, isFree, testDetails } = route.params
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                <View style={[styles.instructionContainer, { backgroundColor: colors.cardBg, }]}>
                    ...
                    <CustomeText color={colors.textClr} style={{ fontWeight: 'bold' }} fontSize={15}>
                        Duration: {testDetails?.test_series_info?.time} Mins
                    </CustomeText>
                    <CustomeText color={colors.textClr} fontSize={13}>
                        Read the following Instructions carefully.
                    </CustomeText>
                    <CustomeText color={colors.textClr} fontSize={13}>
                        1. The test contains {testDetails?.total_questions} total questions
                    </CustomeText>
                    <CustomeText color={colors.textClr} fontSize={13}>
                        2. Each question has options out of which only one is correct.
                    </CustomeText>
                    <CustomeText color={colors.textClr} fontSize={13}>
                        3. You have to finish the test in {testDetails?.test_series_info?.time} minutes
                    </CustomeText>
                    <CustomeText color={colors.textClr} fontSize={13}>
                        4. You will be awarded {(testDetails?.total_marks / testDetails?.total_questions.toFixed(2))} mark for each corect answer and <Text style={{ color: 'red' }}>{testDetails?.test_series_info?.negative_mark}</Text> will be deducted for each wrong answer
                    </CustomeText>
                    <CustomeText color={colors.textClr} fontSize={13}>
            ...
            <View style={{ padding: screenWidth * 4, width: '100%', gap: screenWidth * 2 }}>
                <TouchableOpacity disabled={!isContinue} style={{ backgroundColor: isContinue ? colors.lightBlue : "#ccc", padding: screenWidth * 2, borderRadius: screenWidth * 2, alignItems: 'center', justifyContent: 'center' }}
                    onPress={() => replace('QuizStartScreen', { categoryId, testSeriesId, packgetId, data, total_marks, isFree })}
                >
