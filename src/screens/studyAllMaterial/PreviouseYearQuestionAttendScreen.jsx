...
import { addUserCollectionSlice, attendPreviouseYearQuestionSlice, getPreviouseYearGetQuestionSlice, getUserCollectionDetailSlice, reportedQuestionSlice } from '../../redux/userSlice'
...
const PreviouseYearQuestionAttendScreen = ({ route }) => {
    ...
    // console.log("PreviouseYearQuestionAttendScreen previouseExamId==>", previouseData)
    ...
    const [timeLeft, setTimeLeft] = useState(previouseData?.duration * 60);
    ...
    // ALL STORAGE KEYS
    const skipKey = `previous_skippedQuestions_${userId}_${testId}`;
    const timersKey = `${STORAGE_KEY}_${userId}_${testId}`;
    const progressKey = `previous_test_progress_${userId}_${testId}`;
    const selectedOptionKey = `previous_selectedOption_${userId}_${testId}`;
    const quizOptionKey = `previous_quiz_${userId}_${testId}_options`;
    const markReviewKey = `previous_markedForReview_${userId}_${testId}`;
    const spentTimeKey = `previous_questionTimeSpent_${userId}_${testId}`;
    const test_status_key = "previous_test_status"
    const total_marks = previouseData?.total_question * previouseData?.marks_per_question
    ...
    const fetchPreviousQuestionData = async () => {
        try {
            ...
            const res = await dispatch(getPreviouseYearGetQuestionSlice(testId)).unwrap()
    ...
    const loadTimeFromStorage = (testId, data) => {
        const defaultTimeInSeconds = previouseData?.duration ? previouseData?.duration * 60 : 0;
    ...
    const handleSubmit = async () => {
        ...
        const negativeMark = previouseData?.negative_marks;
        ...
        try {
            ...
            if (res.status_code === 200) {
                ...
                // shivam
                replace("PreviouseYearResultScreen", { data: previouseData });
                                            ...
                                            <View style={{
                                                ...
                                                <View style={{
                                                    ...
                                                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: screenWidth }}>
                                                        ...
                                                        <CustomeText color={colors.wrong}>{previouseData?.negative_marks}</CustomeText>
                                                    </View>
...
export default PreviouseYearQuestionAttendScreen
