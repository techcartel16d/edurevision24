...
import {
    View, Text, TouchableOpacity, ScrollView, ActivityIndicator, Alert, FlatList, StyleSheet
} from 'react-native';
...
const AttemptedTestPage = () => {
    ...
    const formatAttemptedDate = (dateString) => {
        ...
        return `Attempted on ${date.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}`;
    };
    ...
    const handleReattemptClick = (testData) => {
        ...
        navigation.navigate('TestScreen', { testData, isReattempt: true });
    };

    const groupTestsByDate = (tests) => {
        const sortedTests = [...tests].sort((a, b) => parseDate(b.attended_at) - parseDate(a.attended_at));
        const grouped = {};
        const dateOrder = [];
        ...
        <View style={styles.testCard}>
            <View style={styles.testInfo}>
                ...
                <View style={styles.testStats}>
                    ...
                    <Text style={styles.marksText}>✍ {item.marks}/{calculateTotalQuestions(item)} Marks</Text>
                </View>
                <Text style={styles.attemptDate}>{formatAttemptedDate(item.attended_at)}</Text>
                <Text style={styles.detailText}>
                    Correct: {item.correct} | Incorrect: {item.in_correct} | Unattempted: {item.total_not_answer_question}
                </Text>
                <Text style={styles.detailText}>Status: {item.status}</Text>
            </View>
            <View style={styles.actionButtons}>
                <TouchableOpacity style={styles.outlineButton} onPress={() => handleSolutionClick(item)}>
                    <Text style={styles.outlineButtonText}>Solution</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.outlineButton} onPress={() => handleAnalysisClick(item)}>
                    <Text style={styles.outlineButtonText}>Analysis</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.primaryButton} onPress={() => handleReattemptClick(item)}>
    ...
    return (
        <SafeAreaView style={{ flex: 1 }} >
            <CommanHeader heading={"Attenpted Test"} />
            <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 40 }}>
                ...
                {loading ? (
                    ...
                    <View style={styles.noDataContainer}>
                        ...
                        <Text style={styles.noDataSubText}>Start taking tests to see your progress here</Text>
                    </View>
                ) : (
                    dateOrder.map(date => (
...
const styles = StyleSheet.create({
    ...
    noDataText: { fontSize: 18, fontWeight: '600', marginBottom: 8, color: '#374151' },
    noDataSubText: { fontSize: 14, color: '#6B7280' },
    dateGroup: { marginBottom: 24 },
    dateHeader: { fontSize: 20, fontWeight: '600', marginBottom: 12, color: '#111827' },
    testCard: { backgroundColor: 'white', borderRadius: 12, padding: 16, marginBottom: 12,  },
    testInfo: { marginBottom: 12 },
    proBadge: { backgroundColor: '#FBBF24', color: '#92400E', alignSelf: 'flex-start', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 6, marginBottom: 8, fontWeight: '600', fontSize: 12 },
    testTitle: { fontSize: 18, fontWeight: '600', color: '#111827', marginBottom: 6 },
    ...
    outlineButton: { borderWidth: 1, borderColor: '#2563EB', borderRadius: 8, paddingVertical: 8, paddingHorizontal: 12, marginRight: 8 },
    outlineButtonText: { color: '#2563EB', fontWeight: '600' },
    primaryButton: { backgroundColor: '#2563EB', borderRadius: 8, paddingVertical: 8, paddingHorizontal: 16 },
