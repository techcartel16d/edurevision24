import { Modal, RefreshControl, SafeAreaView, ScrollView, StyleSheet, Text, TouchableOpacity, View, ActivityIndicator, Image, Alert, BackHandler } from 'react-native'
...
import { getMegaQuizAttendQuetionGetSlice, megaQuizAttendSubmitSlice } from '../../redux/megaQuizeSlice'
import RenderHTML from 'react-native-render-html'

const MegaQuizeAttendScreen = ({ route }) => {
    const { megaQuizeData } = route.params
    // console.log("PreviouseYearQuestionAttendScreen previouseExamId==>", megaQuizeData)
    ...
    const [timeLeft, setTimeLeft] = useState(megaQuizeData?.duration * 60);
    const [isPaused, setIsPaused] = useState(false);
    const timerRef = useRef(null);
    const STORAGE_KEY = 'previous_test_timers';
    ...
    const testId = megaQuizeData?.id;

    // ALL STORAGE KEYS
    const skipKey = `megaQuize_skippedQuestions_${userId}_${testId}`;
    const timersKey = `${STORAGE_KEY}_${userId}_${testId}`;
    const progressKey = `megaQuize_test_progress_${userId}_${testId}`;
    const selectedOptionKey = `megaQuize_selectedOption_${userId}_${testId}`;
    const quizOptionKey = `megaQuize_quiz_${userId}_${testId}_options`;
    const markReviewKey = `megaQuize_markedForReview_${userId}_${testId}`;
    const spentTimeKey = `megaQuize_questionTimeSpent_${userId}_${testId}`;
    const test_status_key = "megaQuize_test_status"
    const total_marks = megaQuizeData?.total_question * megaQuizeData?.marks_per_question
    ...
    const megaQuizQuetionFetch = async () => {
        try {
            const res = await dispatch(getMegaQuizAttendQuetionGetSlice(testId)).unwrap()
            console.log("PreviouseYearQuestionAttendScreen response", res)
    ...
    const loadTimeFromStorage = (testId, data) => {
        const defaultTimeInSeconds = megaQuizeData?.duration ? megaQuizeData?.duration * 60 : 0;
    ...
    const handleSubmit = async () => {
        ...
        const negativeMark = megaQuizeData?.negative_marks;
        ...
        try {
            ...
            if (res.status_code === 200) {
                ...
                replace("MegaQuizeResultScreen", { data: megaQuizeData });
    ...
    const handleExitQuiz = async () => {
        ...
        // setIsPaused(true); // Optional: if you want to pause the quiz logic
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }}>
            <View style={[styles.quizHeader, { backgroundColor: colors.lightGray }]}>
                <View style={{
                    ...
                    <TouchableOpacity onPress={handleExitQuiz}>
                    ...
                    <View style={{
                        ...
                        <CustomeText fontSize={10} color={colors.textClr}>{megaQuizeData?.test_series_type}</CustomeText>
                    </View>
                </View>
                <View style={{
                    flexDirection: 'row',
                ...
                <View style={{
                    ...
                    <MaterialCommunityIcons name="flag-outline" size={RFValue(20)} color={colors.textClr} />
                                        ...
                                        <View style={{
                                            ...
                                            <View style={{
                                                ...
                                                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: screenWidth }}>
                                                    ...
                                                    <CustomeText color={colors.wrong}>{megaQuizeData?.negative_marks}</CustomeText>
                                                </View>
                                        ...
                                        <View color={markedForReview.includes(previousQuestions[currentQuestion]?.id) ? 'white' : colors.textClr}>
                                            {
                                                language === 'English' ? (
                                                    <RenderHTML
                                                        ...
                                                        source={{
                                                            html: `<p>${previousQuestions[currentQuestion]?.question_english}</p>` || '<p>hello</p>'
                                                        }}
                                                        tagsStyles={{
                                                    ...
                                                    <RenderHTML
                                                        ...
                                                        contentWidth={fullWidth}
                                                        source={{
                                                            html: `<p>${previousQuestions[currentQuestion]?.question_hindi}</p>` || '<p>hello</p>'
                                                        }}
                                                        tagsStyles={{
                                                            p: { color: colors.textClr },
                                                            div: { color: colors.textClr },
...
export default MegaQuizeAttendScreen
